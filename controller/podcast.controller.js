...
const addPodcast = async (req, res) => {
  try {
    const { podcastTitle, authorName, description } = req.body;
    ...
    if (req.files && req.files["videoFile"]) {
      ...
      if (req.files.videoFile[0]) {
        ...
        newPodcast.podcastVideo = videoFileName;
      }
    }
    await newPodcast.save();
    return res
      .status(HTTP_STATUS.CREATED)
      .send(success("podcast added successfully", newPodcast));
  } catch (err) {
    return res
...
const disableServiceById = async (req, res) => {
  try {
    ...
    const service = await Podcast.findByIdAndUpdate(
      req.params.id,
      { isDisabled: true },
      { new: true }
    );
